...
const AdminManageGestores: React.FC = () => {
  ...
  const confirmDelete = () => {
    if (gestorToDelete) {
      setGestores(prev => prev.map(g => g.id === gestorToDelete.id ? {
        ...g,
        estado: "Inactivo",
  ...
  return <AdminLayout showSidebar={true} sidebarOpen={sidebarOpen} onSidebarToggle={() => setSidebarOpen(!sidebarOpen)}>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          ...
          <Button onClick={() => navigate("/admin/gestores/create")} className="gap-2">
          ...
          <Table>
            ...
            <TableBody>
              {gestores.map(gestor => <TableRow key={gestor.id}>
                    ...
                    <TableCell>
                      <span className={`px-2 py-1 rounded-full text-xs ${gestor.estado === "Activo" ? "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200" : "bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300"}`}>
                        {gestor.estado}
                      </span>
                    </TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button variant="ghost" size="icon" onClick={() => navigate(`/admin/gestores/edit/${gestor.id}`)} aria-label="Editar gestor">
                          <Edit className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => handleDelete(gestor)} aria-label="Eliminar gestor">
